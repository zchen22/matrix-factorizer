...
__global__ void SgdFeatureHalf(const int* __restrict__ user_ids,
  ...
  const float user_feature = __half2float(user_features[user_feature_idx]);
  const float item_feature = __half2float(item_features[item_feature_idx]);
  ...
  user_features[user_feature_idx] = __float2half_rn(new_user_feature);
  item_features[item_feature_idx] = __float2half_rn(new_item_feature);
}
