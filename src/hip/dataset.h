...
class Dataset {
 public:
  ...
  ~Dataset();
  // Getters
  int GetUserId(const int record_id) const { return record_vector_[record_id].user_id; }
  int GetItemId(const int record_id) const { return record_vector_[record_id].item_id; }
  float GetRating(const int record_id) const { return record_vector_[record_id].rating.f32; }
  ...
  const int16_t* GetRatingHalfDev() const { return rating_half_vector_dev_; }
  const int* GetUserRecordBaseDev() const { return user_record_base_vector_dev_; }
  const int* GetUserNumRecordsDev() const { return user_num_records_vector_dev_; }
  // Load records
  int Load();
  int LoadFromText();
  int LoadFromBinary();
 ...
 private:
  ...
  std::vector<int16_t> rating_half_vector_;
  // User information
  ...
  int16_t* rating_half_vector_dev_;
