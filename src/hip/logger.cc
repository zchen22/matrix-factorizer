...
int Logger::CheckHipError(const hipError_t e) {
  if (e != hipSuccess) {
    Error(stderr, "%s\n", hipGetErrorString(e));
  }
  return 0;
...
int Logger::StopTimer() {
  gettimeofday(&end_time_, nullptr);
  return 0;
}

double Logger::ReadTimer() const {
  return (1. * (end_time_.tv_sec * 1e6 + end_time_.tv_usec) - (start_time_.tv_sec * 1e6 + start_time_.tv_usec)) / 1e6;
}
