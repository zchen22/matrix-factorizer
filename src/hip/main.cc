...
int ParseCommandArgs(const int argc, const char** argv, std::unordered_map<std::string, std::string>& arg_map) {
...
int PrintHelp(const char* exec) {
  fprintf(stderr, "\nUsage: %s -t train-file [-e test-file] [-c config-file] [-d gpu-id]\n\n", exec);
  fprintf(stderr, "-t train-file:\n");
  fprintf(stderr, "\tTrain data file in the COO format.\n");
  fprintf(stderr, "\tThe first line is a (number-of-users, number-of-items, number-of-ratings) tuple, where the elements are divided by spaces/tabs.\n");
  fprintf(stderr, "\tEach following line is a (user-id, item-id, rating) tuple, where the elements are divided by spaces/tabs.\n");
  fprintf(stderr, "\tComment lines start with '%%'s.\n");
  fprintf(stderr, "[-e test-file]:\n");
  fprintf(stderr, "\tTest data file in the COO format.\n");
  fprintf(stderr, "\tThe first line is a (number-of-users, number-of-items, number-of-ratings) tuple, where the elements are divided by spaces/tabs.\n");
  fprintf(stderr, "\tEach following line is a (user-id, item-id, rating) tuple, where the elements are divided by spaces/tabs.\n");
  fprintf(stderr, "\tComment lines start with '%%'s.\n");
  ...
  fprintf(stderr, "\tGPU used to accelerate computation. If skipped, CPU is used.\n");
  return 0;
}
